    ...
    public User getUserByname(String username) {
        return jdbcTemplate.queryForObject(FETCH_BY_USERNAME, new Object[]{username}, new RowMapper<User>() {
            ...
            public User mapRow(ResultSet resultSet, int i) throws SQLException {
            	 List<GrantedAuthority> grAuthsList = new ArrayList<GrantedAuthority>();
            	 grAuthsList.add(new SimpleGrantedAuthority("ROLE_USER"));
                 User user = new User(resultSet.getString("username"), resultSet.getString("password"),  grAuthsList);
                 return user;
            }
        });
